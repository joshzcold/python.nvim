...
function PythonVENVCreate.delete_venv_from_state(venv_key, delete_dir)
  if not venv_key then
    vim.notify_once(
      string.format("python.nvim: Could not delete venv from state. venv_key was nil"),
...
function PythonVENVCreate.python_set_venv(venv_path, venv_name, venv_source)
  local lsp = require("python.lsp")
  if venv_path then
    local python_venv = require("python.venv")
...
function PythonVENVCreate.uv_sync(uv_lock_path, venv_dir, callback, script)
...
---@param venv_dir string full path to pdm lock file
---@param callback function
function PythonVENVCreate.pdm_sync(pdm_lock_path, venv_dir, callback)
...
---@param venv_dir string full path to pdm lock file
---@param callback function
function PythonVENVCreate.poetry_sync(poetry_lock_path, venv_dir, callback)
  ...
  vim.system({ "poetry", "env", "use", vim.fs.joinpath(venv_dir, "bin", "python") }, {
    cwd = dir_name,
  }, function(obj1)
    if obj1.code ~= 0 then
      vim.notify_once("python.nvim: " .. vim.inspect(obj1.stderr), vim.log.levels.ERROR)
    ...
    vim.system({ "poetry", "sync", "--no-root" }, {
      cwd = dir_name,
      stdout = ui.show_system_call_progress,
    }, function(obj2)
      vim.schedule(function()
...
---@param callback function
function PythonVENVCreate.pip_install_with_venv(requirements_path, venv_dir, callback)
...
function PythonVENVCreate.create_venv_with_python(venv_path, python_interpreter)
...
local function pick_python_interpreter()
  ...
  vim.schedule(function()
    vim.ui.select(interpreters.python_interpreters(), { prompt = "Select a python interpreter: " }, function(str)
...
local function pick_venv_path(detect)
  ...
  if vim.fn.isdirectory(wanted_dir) == 0 then
    vim.notify_once(
      string.format("Error: directory of new venv doesn't exist: '%s'", venv_path_user_input),
      vim.log.levels.ERROR
    )
    return
  end
...
local function venv_install_file(detect)
  ...
  coroutine.resume(coroutine.create(function()
    ...
    install_function(detect.venv.install_file, venv_path_user_input, function()
      ...
      PythonVENVCreate.python_set_venv(val.venv_path, venv_name)
...
local function set_venv_state()
  ...
  if detect == nil then
    vim.notify("python.nvim: Could not find a python dependency file for this cwd", vim.log.levels.WARN)
    return
  end
...
function PythonVENVCreate.create_and_install_venv()
  set_venv_state()
end
...
local function delete_venv_from_selection()
  ...
  vim.ui.select(keys, {
    ...
    PythonVENVCreate.delete_venv_from_state(choice, true)
  end)
end

---Delete a venv from state and filesystem
---@param select boolean
function PythonVENVCreate.delete_venv(select)
  local detectM = require("python.venv.detect")
  if select then
  ...
  PythonVENVCreate.delete_venv_from_state(detect.dir, true)
end
...
function PythonVENVCreate.detect_contents_for_dependencies(_, _)
  local detectM = require("python.venv.detect")
